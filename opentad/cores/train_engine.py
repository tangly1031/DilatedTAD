...
def train_one_epoch(
    ...
    logger.info(f"Training time for epoch {curr_epoch}: {training_time:.2f} seconds")
...
def val_one_epoch(
    ...
    #logger.info("  ".join([block1, block2, "  ".join(block3), "Avg Inference Time={:.4f} seconds".format(avg_inference_time)]))
