...
class FocalEIoULoss(nn.Module):
    ...
    def forward(self, pred_boxes, target_boxes, reduction='mean', eps=1e-6):
        """
        Calculate Focal Enhanced IoU Loss.

        Args:
            pred_boxes (torch.Tensor): Predicted boxes of shape (N, 2) [x1, x2].
            target_boxes (torch.Tensor): Target boxes of shape (N, 2) [x1, x2].
            reduction (str): Reduction method, either 'mean' or 'sum'.
            eps (float): Small number to prevent division by zero.

        Returns:
            torch.Tensor: Computed Focal EIoU loss.
        """
        # Calculate areas
        pred_area = (pred_boxes[:, 1] - pred_boxes[:, 0]).clamp(min=0)
        target_area = (target_boxes[:, 1] - target_boxes[:, 0]).clamp(min=0)

        # Calculate intersection
        inter_max = torch.min(pred_boxes[:, 1], target_boxes[:, 1])
        inter_min = torch.max(pred_boxes[:, 0], target_boxes[:, 0])
        ...
        enclosing_max = torch.max(pred_boxes[:, 1], target_boxes[:, 1])
        enclosing_min = torch.min(pred_boxes[:, 0], target_boxes[:, 0])
        enclosing_area = (enclosing_max - enclosing_min).clamp(min=0)

        # Calculate center distance
        pred_center = (pred_boxes[:, 0] + pred_boxes[:, 1]) / 2
        target_center = (target_boxes[:, 0] + target_boxes[:, 1]) / 2
        center_distance = (pred_center - target_center).pow(2)

        # Calculate EIoU
        eiou = iou - (center_distance / (enclosing_area + eps))
        ...
        if reduction == 'mean':
            ...
            return focal_eiou_loss.sum()
        else:
            return focal_eiou_loss

# Usage
# focal_eiou_loss = FocalEIoULoss()
# loss = focal_eiou_loss(pred_boxes, target_boxes)
...
class EIoULoss(nn.Module):
    ...
    def forward(self, pred_boxes, target_boxes, reduction='mean', eps=1e-6):
        """
        Calculate the Enhanced IoU Loss.
        ...
        Returns:
            torch.Tensor: Computed EIoU loss
        """
        # Calculate areas
        pred_area = (pred_boxes[:, 1] - pred_boxes[:, 0]).clamp(min=0)
        target_area = (target_boxes[:, 1] - target_boxes[:, 0]).clamp(min=0)
