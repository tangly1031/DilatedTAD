        ...
        for mn, m in self.named_modules():
            for pn, p in m.named_parameters():
                ...
                if pn.endswith("bias"):
                    ...
                    no_decay.add(fpn)
                elif "cross_attention" in fpn and pn.endswith("in_proj_weight"):
                    # cross_attention µÄ in_proj_weight 
                    decay.add(fpn)
                elif (
                    pn.endswith("A_log")
                    or pn.endswith("D_b")
                    ...
                    or pn.endswith("in_proj.weight")
                    or pn.endswith("conv1d.weight")
                    or pn.endswith("conv1d_b.weight")
                    or pn.endswith("out_proj.weight")
                ):
                    # corner case for mamba
                    decay.add(fpn)
                elif pn.endswith("norm.weight") or pn.endswith("gamma") or pn.endswith("dt_bias") or pn.endswith("dt_bias_b"):
                    # LayerNorm and GroupNorm weights will not be decayed
                    no_decay.add(fpn)
